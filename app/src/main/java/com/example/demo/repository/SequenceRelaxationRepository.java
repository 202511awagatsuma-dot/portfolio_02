...
@Repository
public class SequenceRelaxationRepository {

    private final JdbcTemplate jdbcTemplate;

    public SequenceRelaxationRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    public List<SequenceRelaxation> findBySequenceId(Long sequenceId) {
        return jdbcTemplate.query(
                """
                SELECT sr.id,
                       sr.sequence_id,
                       sr.relaxation_master_id,
                       sr.custom_name,
                       COALESCE(rm.name_ja, sr.custom_name) AS display_name,
                       sr.sort_order,
                       sr.memo,
                       sr.created_at,
                       sr.updated_at
                FROM sequence_relaxation sr
                LEFT JOIN relaxation_master rm ON rm.id = sr.relaxation_master_id
                WHERE sr.sequence_id = ?
                ORDER BY sr.sort_order ASC, sr.id ASC
                """,
                (rs, rowNum) -> new SequenceRelaxation(
                        rs.getLong("id"),
                        rs.getLong("sequence_id"),
                        getNullableLong(rs, "relaxation_master_id"),
                        rs.getString("custom_name"),
                        rs.getString("display_name"),
                        rs.getInt("sort_order"),
                        rs.getString("memo"),
                        rs.getObject("relaxation_master_id") == null,
                        rs.getTimestamp("created_at").toLocalDateTime(),
                        rs.getTimestamp("updated_at").toLocalDateTime()),
                sequenceId);
    }

    public Long addMasterSelection(Long sequenceId, Long relaxationMasterId, String memo) {
        return insert(sequenceId, relaxationMasterId, null, memo);
    }
    ...
    public void delete(Long id, Long sequenceId) {
        jdbcTemplate.update(
                """
                DELETE FROM sequence_relaxation
    ...
    private Long insert(Long sequenceId, Long relaxationMasterId, String customName, String memo) {
        Integer nextSortOrder = jdbcTemplate.queryForObject(
                """
                SELECT COALESCE(MAX(sort_order), 0) + 1
                FROM sequence_relaxation
                WHERE sequence_id = ?
                """,
                Integer.class,
                sequenceId);
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement statement = connection.prepareStatement(
                    """
                    INSERT INTO sequence_relaxation
                    (sequence_id, relaxation_master_id, custom_name, sort_order, memo, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                    """,
                    new String[] { "id" });
            statement.setLong(1, sequenceId);
            if (relaxationMasterId == null) {
                statement.setNull(2, java.sql.Types.BIGINT);
            } else {
                statement.setLong(2, relaxationMasterId);
            }
    ...
    private Long extractGeneratedId(KeyHolder keyHolder) {
        ...
        throw new IllegalStateException("Failed to extract generated sequence_relaxation id.");
    }
